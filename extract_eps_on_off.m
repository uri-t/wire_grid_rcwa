function [eps_on] = extract_eps_on_off(tf, freq, eps_off, eps_wires, eps_subs, f, d, period, N)

eps_on = zeros(size(freq));

eps_start = [9.01, -0.001];

for ii = 1:numel(freq)
    err = @(eps) n_error(tf(ii),...
                         rcwa_on_off(freq(ii), eps_off, eps(1)+1i*eps(2), eps_wires(ii), eps_subs, f, d, period, N));

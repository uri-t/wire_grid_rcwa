...
def parse_fname(fname):
    ...
    wires = not bool(re.search('no_wires', fname))
    return {'grid': vals[0], 'np_size': vals[1], 'wires': wires}

# get all np simulations
fnames = glob.glob('simulation_data/effective_medium/parsed/*100.txt')
...
for fname in fnames:
    base = fname[:-7]
    tf = get_tf(base + "100.txt", base + "0.txt")
    params = parse_fname(fname)
        
    if params['wires']:
        cond = analyze_wires(tf)
    else:
        cond = analyze_no_wires(tf)
    ...
    df_curr = pd.DataFrame.from_dict({
        ...
        'freq': freq,
        'grid': params['grid'],
        'np_size': params['np_size'],
        'wires': params['wires']
        })

    df = pd.concat([df, df_curr])

df.to_csv('np_effective_medium.csv')
